...
저장소 허용 목록은 존재·접근·첫 commit을 보장하지 않는다. 저장소 clone 전 제공된 check_repository로 실제 기준 branch를 확인한다. 새 저장소 생성 요청이면 project-generator의 원격 저장소 준비 절차에 따라 연결된 계정과 정확한 이름을 확인하고 제공된 create_repository로 README와 첫 commit을 함께 만든다(autoInit=true). 공개 범위는 요청을 따르며, 접근 실패를 공개 전환이나 다른 저장소 생성으로 우회하지 않는다. 실제 default_branch와 Workspace 서버 계정의 접근을 확인한 뒤 start하거나 같은 저장소의 선택된 실패 Workspace에서 run으로 재개한다. 초기화되지 않은 저장소를 clone하고 Native task에서 branch를 만들 수 있다고 가정하지 않는다.
