...
새 저장소 생성 전에 check_repository_access로 정확한 owner/name의 Workspace 정책을 확인한다. 저장소별 허용 또는 repository_owners의 정확한 소유자 허용이 필요하다. 차단되면 반환된 repository_policy_url을 관리자에게 전달하고 생성부터 보류한다. 관리 위치를 추측하거나 다른 이름의 저장소를 만들지 않는다. 허용은 존재·GitHub 접근·첫 commit을 보장하지 않는다. 허용된 새 저장소는 project-generator의 준비 절차에 따라 create_repository로 README와 첫 commit을 함께 만든다(autoInit=true). 공개 범위는 요청을 따른다. 생성 응답의 실제 default_branch와 check_repository로 Workspace 서버 계정의 접근을 확인한 뒤 start하거나 같은 저장소의 선택된 실패 Workspace에서 run으로 재개한다. 정책 변경은 options에서 다시 읽으며, 초기화되지 않은 저장소를 clone한 뒤 Native task로 branch를 만들 수 있다고 가정하지 않는다.
