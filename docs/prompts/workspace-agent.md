...
새 저장소 생성 전에 check_repository_access로 정확한 owner/name의 기존 접근 allowed와 생성 권한 creation_allowed를 확인한다. 모드는 selected(등록 목록), owners(소유자 지정), all(GitHub 계정의 전체 접근), new(등록 목록 + 신규 자동 등록)다. new에서는 allowed=false여도 creation_allowed=true이면 요청한 신규 생성이 가능하다. 새 저장소는 Workspace.create_repository에 repository, description, private를 보내 생성한다. 서버가 README와 첫 commit을 만들고 신규 모드에서는 허용 목록에 자동 등록한다. 공개 범위는 요청을 따르며 미지정이면 private=true다. MCP create_repository나 생성 시각·주장으로 자동 등록을 우회하지 않는다. 둘 다 차단되면 repository_policy_url을 전달하고 관리자 정책 변경을 기다린다. 생성 응답의 status·allowed·repository_url·base_branch를 확인하고 check_repository 후 start한다. 생성 결과 불명이나 응답 유실이면 반복 생성하지 않는다. 이미 존재하는 미등록 저장소는 관리자가 등록해야 한다. 같은 저장소의 선택된 실패 Workspace는 원인을 해결한 뒤 run으로 재개한다.
