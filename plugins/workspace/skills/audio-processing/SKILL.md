...
3. 완료된 작업의 task·sourceIdentity·artifacts 관계와 현재 파일 사용 가능 여부를 확인한다.
   ...
   미완료 후속 단계가 있고 입력 파일을 사용할 수 있으면 그 단계만 제출한다.
   완료된 단계와 만료·삭제된 원본을 자동 재처리하지 않는다. 명시적 재처리 요청에는 새 processing_revision을 사용한다.
...
6. 제출 응답의 status는 접수 결과이고 job.status는 실행 상태다. job.status가 completed이면
   stage가 cleaning이어도 이미 끝난 작업이다. 접수·중복·과거 완료 결과의 재사용을 구분해 job ID와 시각을 보고한다.
   사용 가능한 원본·전사·대화·요약 Artifact만 안내하고 링크는 도구가 반환한 값을 사용한다.
