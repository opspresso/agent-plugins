...
description: >
  ...
  없이 혼자 열리는 HTML 한 개를 코드 블록으로 만들고, 편집 지면처럼 여백과 활자로
  읽히게 한다. "리포트 만들어줘", "HTML로 뽑아줘", "결과를 보기 좋게 정리해줘"가
...
**혼자 열리는 HTML 하나**를 쓴다. CSS와 JS를 전부 인라인해서, 사용자가 그대로 저장해
브라우저로 열거나 PDF로 인쇄하면 끝나야 한다. 저장은 사용자가 한다 — 이 런타임에는
파일을 만들 방법이 없다.
...
완성된 HTML 전문을 `html` 코드 블록 하나로 준다. 사용자가 `.html`로 저장해서 열면 된다고
알리고, **파일을 만들었다거나 저장했다고 말하지 않는다.** 이 런타임에는 파일을 만들 방법이
없다. DOCX·PPTX·PDF·HWPX 파일로 건네야 하는 문서는 이 스킬이 아니라 `document-authoring`이다.
