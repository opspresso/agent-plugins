...
name: html-report
description: >
  데이터·메모·조사 결과를 브라우저에서 읽고 인쇄할 HTML 보고서로 구성한다.
  산문·표·차트·출처를 외부 의존성 없는 파일 하나에 담는다.
  형식 미지정 보고서 파일이나 DOCX·PPTX·PDF·HWPX 요청은 document-authoring을 쓴다.
compatibility: >
...
3. **템플릿에서 시작한다.** `references/template.md`의 HTML을 복사해서 내용을 채운다.
   ...
   다시 짜지 않는다.
4. **구조를 잡는다.** 아래 골격에서 자료와 목적에 필요한 절을 고른다.
5. **시각화를 만든다.** 기본은 인라인 SVG다. 라이브러리를 쓰면 필요한 코드를 파일 안에
   포함하고 외부 요청 없이 동작하게 한다.
6. **검토하고 낸다.** 수치·출처·본문 링크를 대조한다. 브라우저 도구가 있으면 좁은 화면,
   키보드 조작, JavaScript 비활성 상태와 인쇄를 확인하고 미검증 조건을 밝힌다.
...
제목과 핵심 내용을 먼저 배치한다. 출처는 실제 사용한 자료로 표시하고 수치가 없으면
핵심 수치·차트를 생략한다. 사용자 양식이 있으면 우선하며 예시의 날짜·지표·작성자를 복사하지 않는다.
...
- **본문 단은 언어와 글꼴에 맞춘다.** 라틴 문자 65~75자, 한국어 40~45자 안팎을
  시작점으로 확인한다. `ch`는 실제 글자 수가 아니며 차트와 표는 더 넓게 둘 수 있다.
...
- **기본은 인쇄에 맞춘 단일 테마다.** 요청에 화면용 다크 테마가 있으면 지원하고
  인쇄에서는 흰 지면과 읽을 수 있는 색을 별도로 지정한다.
- **웹폰트를 링크하지 않는다.** 시스템 스택을 쓰고 문서 언어에 맞는 `lang`을 선언한다.

## 상호작용은 읽기를 돕는 것만

- 목차 스크롤 추적과 부드러운 이동
- 표 정렬·필터 (독자가 실제로 비교·탐색해야 할 때)
