...
장애 보고서는 읽는 사람이 판단과 승인을 하는 문서다. 관측·해석·미확인 구분이 문장에서도
드러나야 하고 AI 티가 나면 판정 자체를 의심받는다.

- **관측은 단언, 미확인은 "확인 못 함".** 확인한 사실에 "~것으로 보입니다", "~것으로
  판단됩니다"를 붙이지 않는다. 확인하지 못한 것은 흐리지 말고 "확인 못 함 — 필요한 조회:
  ~"로 적는다. 해석은 "관측 X 로 보아 Y"처럼 근거를 앞에 두고 단언한다.
- **결론은 한 문장, 대구 없이.** "단순히 A가 아니라 B" 대신 B 를 쓴다. 원인이 둘이면 둘 다
  적고 각각 근거를 붙인다.
- **정도 표현은 수치로.** "심각한", "대대적인", "급격한", "상당한", "매우" 대신 값·비율·시간
  범위를 쓴다. "에러가 급증" → "5xx 가 09:12부터 0.3%에서 4.1%로".
- **"개선", "안정화", "대응", "조치"는 무엇을 하는지로.** 해결 방안은 리소스와 값이 보이게
  적는다. "설정 최적화" → "`maxSurge` 를 25%에서 1로".
- **"~할 필요가 있다", "~해야 한다"를 늘어놓지 않는다.** 방안마다 주체와 조건을 붙인 동사로
  쓴다. "재발 방지가 필요합니다" → "결제 서비스에 `PodDisruptionBudget` 을 추가한다".
- **연결어미 뒤 쉼표를 찍지 않는다.** "-고,", "-지만,", "-면서," 뒤 쉼표는 뺀다. 타임라인은
  한 줄에 사건 하나라서 원래 쉼표가 필요 없다.
- **"결론적으로", "종합하면", "다음과 같은", "이를 통해"를 쓰지 않는다.** 섹션 제목이 이미
  그 역할을 한다. 문두 "또한", "따라서", "이는 ~"도 반복하지 않는다.
- **번역투를 피한다.** "~에 대해 조회" → "~를 조회", "~에 의해 재시작되었다" → "~가 재시작했다",
  "OOM 이 발생하게 되었다" → "OOM 이 났다".
- **서식은 템플릿대로만.** 이모지, 본문 볼드, "X: Y" 콜론 부제, 대시(—) 남용을 쓰지 않는다.
  로그·리소스 이름·값은 백틱으로만 표시한다.
- **챗봇 문구를 남기지 않는다.** "다음은 조사 결과입니다", "도움이 되셨기를", "추가로 궁금한
  점이 있으면". 보고서는 결론으로 시작해서 다음 확인으로 끝난다.

Kubernetes·Prometheus·Loki 같은 도구 이름과 리소스 종류, `OOMKilled` 같은 상태 값은 원어로
둔다. 전체 패턴 목록은 이 저장소의 `plugins/workspace/skills/korean-humanize/ai-tell-catalog.md`.
