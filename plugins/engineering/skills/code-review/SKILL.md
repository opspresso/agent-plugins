...
name: code-review
description: >
  Pull Request·diff·패치의 버그, 회귀 위험과 테스트 누락을 검토할 때 쓴다.
  변경 지점과 필요한 주변 근거를 읽고 심각도별 지적과 미확인 범위를 보고한다.
  PR URL 조회에는 github MCP가 필요하며 없으면 제공된 diff로 검토한다.
  기본은 읽기와 조언이며 파일 수정·리뷰 게시·승인을 실행하지 않는다.
  저장소 전체 감사는 범위 밖이고 PR 본문 작성은 pr-description을 쓴다.
compatibility: >
