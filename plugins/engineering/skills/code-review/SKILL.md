...
name: code-review
description: >
  Pull Request·Merge Request·diff·패치의 버그, 회귀 위험, 보안 문제와 테스트 누락을 검토할 때 쓴다.
...
| 사용자가 준 것 | 하는 일 |
|---|---|
| PR·MR 번호나 URL | 해당 저장소 서비스의 도구로 메타데이터와 변경 파일, diff를 읽는다 |
...
있으므로 설치한 플러그인만 보고 부재를 단정하지 않는다. 현재 런에서 대상 저장소를
읽을 도구가 없으면 diff를 요청한다. 없는 서버나 도구 이름을 만들어내지 않는다.

## 2. 변경 읽기

연결된 저장소 서비스나 로컬 읽기 도구의 실제 schema를 확인한다. 특정 호스팅 제품을 전제하지 않는다.

1. PR의 제목·본문·상태·base/head를 읽는다.
2. 변경 파일 목록과 파일별 추가·삭제 줄 수를 읽는다.
3. diff를 읽는다. 너무 크면 파일 단위로 좁힌다.
4. 판단에 필요할 때만 원본 파일을 연다.
